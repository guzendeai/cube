...
export default function Cube3D({ onFaceClick, size = 340 }: Cube3DProps) {
  ...
  const runInertia = useCallback(() => {
    ...
    setRotX(prev => prev + velocityRef.current.x);
    setRotY(prev => prev + velocityRef.current.y);

    const nextRotX = rotX + velocityRef.current.x;
    const nextRotY = rotY + velocityRef.current.y;
    updateHiddenFace(nextRotX, nextRotY);
